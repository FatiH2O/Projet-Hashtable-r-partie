...
public class Front extends AbstractComponent{
	...
	Composite_Endpoint Front_to_Node_edp;

	
	protected Front(DHTServices_Simple_endpoint Client_Front_edp,Composite_Endpoint  Front_to_Node_edp) throws ConnectionException {
		super(1, 0);
	...
	public ContentDataI get(ContentKeyI key) throws Exception {
		...
		return this.Front_to_Node_edp.getAccessSync_edp().getClientSideReference().getSync(URIGenerator.generateURI(), key);
	}

	
	public ContentDataI put(ContentKeyI key, ContentDataI value) throws Exception {
		
		return this.Front_to_Node_edp.getAccessSync_edp().getClientSideReference().putSync(URIGenerator.generateURI(), key, value);
	}

	
	public ContentDataI remove(ContentKeyI key) throws Exception {
		
		return this.Front_to_Node_edp.getAccessSync_edp().getClientSideReference().removeSync(URIGenerator.generateURI(), key);
	}
	...
	public <R extends Serializable, A extends Serializable> A mapReduce(SelectorI selector, ProcessorI<R> processor,
		...
		this.Front_to_Node_edp.getMapreduce_edp().getClientSideReference().mapSync(Uri, selector, processor);
		return this.Front_to_Node_edp.getMapreduce_edp().getClientSideReference().reduceSync(Uri, reductor, combinator, initialAcc);
	}
