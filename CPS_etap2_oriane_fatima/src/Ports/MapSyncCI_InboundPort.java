...
public class MapSyncCI_InboundPort extends  AbstractInboundPort implements MapReduceSyncCI{
	...
	public <R extends Serializable> void mapSync(String computationURI, SelectorI selector, ProcessorI<R> processor)
			throws Exception {
		this.getOwner().handleRequest(owner-> {((Node)owner).mapSync(computationURI, selector, processor); 
		return null;});
	...
	public void clearMapReduceComputation(String computationURI) throws Exception {
		this.getOwner().handleRequest(owner-> {((Node)owner).clearMapReduceComputation(computationURI); 
		return null;});
	}
